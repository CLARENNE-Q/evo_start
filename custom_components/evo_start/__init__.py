...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    hass.async_create_task(
        hass.config_entries.async_forward_entry_setups(entry, ["sensor", "lock", "device_tracker"])
    )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok := await hass.config_entries.async_unload_platforms(entry, ["sensor", "lock", "device_tracker"]):
